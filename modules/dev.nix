  ...
  home.packages = with pkgs; [
    ...
    (writeShellScriptBin "ns" ''
      ${nix-search-tv}/bin/nix-search-tv print | ${fzf}/bin/fzf \
        ...
        --scheme history \
        --bind 'ctrl-o:execute(xdg-open "https://search.nixos.org/packages?channel=unstable&query=$(echo {} | awk "{print \$1}" | sed "s|^.*/||")")' \
        --bind 'ctrl-s:execute(xdg-open "https://github.com/search?q=repo%3ANixOS%2Fnixpkgs+$(echo {} | awk "{print \$1}" | sed "s|^.*/||")&type=code")' \
        --bind 'ctrl-i:execute(${pkgs.kitty}/bin/kitty -e nix shell nixpkgs#$(echo {} | awk "{print \$1}" | sed "s|^.*/||"))' \
        --header 'Ctrl-o: web | Ctrl-s: github | Ctrl-i: shell | Enter: copy' \
        --bind 'enter:execute(echo {} | awk "{print \$1}" | sed "s|^.*/||" | ${pkgs.wl-clipboard}/bin/wl-copy)+abort'
    '')
